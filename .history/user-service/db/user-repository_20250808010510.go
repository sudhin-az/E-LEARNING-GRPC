...
func (r *UserRepository) CreateUser(ctx context.Context, userID, name, email string) (User, error) {
	id := uuid.NewString()
	...
	err := r.conn.Create(&newUser).Error
	if err != nil {
		return User{}, fmt.Errorf("failed to create user: %v", )
	}
}
